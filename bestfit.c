...
void main()
{
    ...
    // Best fit: i -> process, j -> block
    while (i < n)
    {
        int best_fit_index = -1;
        int min_free_space = 99999; // Set to a large number initially

        if (p[i].alloc == 0)
        {
            for (j = 0; j < m; j++)
            {
                // Find the block with minimum wasted space for the process
            ...
            {
                // If no suitable block is found, print "NOT ALLOCATED"
